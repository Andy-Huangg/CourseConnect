...
export function usePrivateMessageSocket(
  ...
  const connectWebSocket = useCallback(() => {
    ...
    if (isConnectingRef.current) {
      console.log("Private message connection already in progress, skipping...");
      return;
    }
    ...
    if (socketRef.current) {
      ...
      if (currentState !== WebSocket.CLOSED && currentState !== WebSocket.CLOSING) {
        console.log("Closing existing private message connection before creating new one");
    ...
    socketRef.current.onclose = (event) => {
      ...
      if (event.code !== 1000 && reconnectAttemptsRef.current < 5) {
        ...
        console.log(`Attempting to reconnect private message WebSocket in ${delay}ms (attempt ${reconnectAttemptsRef.current + 1})`);
